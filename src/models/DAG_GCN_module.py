...
class GCNEncoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, adj_A):
        ...
        coo = coo_matrix(adj_A)
        self.edge_index = torch.tensor(np.array([coo.row, coo.col]), dtype=torch.long)
        self.Wa = nn.Parameter(torch.zeros(out_channels), requires_grad=True)
        
        self.gcn_shared = pyg.nn.Sequential('x, edge_index', [
            (pyg.nn.DenseGCNConv(in_channels, hidden_channels), 'x, edge_index -> x'),
            nn.ReLU(inplace=True),
        ])
        
        self.gcn_mu = pyg.nn.Sequential('x', [
            (pyg.nn.DenseGCNConv(hidden_channels, out_channels), 'x, edge_index -> x'),
        ])
        
        self.gcn_logvar = pyg.nn.Sequential('x', [
            (pyg.nn.DenseGCNConv(hidden_channels, out_channels), 'x, edge_index -> x'),
        ])
    ...
    def forward(self, x):
        x = self.gcn_shared(torch.reshape(x,(11,-1)), self.adj_A)
        # x = self.gcn_shared(torch.reshape(x,(512,11)), self.edge_index.type_as(x))

        adj_A = pyg.utils.to_dense_adj(self.edge_index)
        adj_A1 = torch.sinh(3.0 * adj_A).type_as(x)
        
        adj = self.adj_process(adj_A1)
        
        mu = self.gcn_mu(x, self.edge_index)
        mu = torch.matmul(adj, mu + self.Wa) - self.Wa
        
        logvar = self.gcn_logvar(x, self.edge_index)
...
class GCNDecoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels):
        ...
        self.gcn_shared = pyg.nn.Sequential('x, edge_index', [
            (pyg.nn.GCNConv(in_channels, hidden_channels), 'x, edge_index -> x'),
            nn.ReLU(inplace=True),
        ])
        
        self.gcn_mu = pyg.nn.Sequential('x, edge_index', [
            (pyg.nn.GCNConv(hidden_channels, out_channels), 'x, edge_index -> x'),
        ])
        
        self.gcn_logvar = pyg.nn.Sequential('x, edge_index', [
            (pyg.nn.GCNConv(hidden_channels, out_channels), 'x, edge_index -> x'),
        ])
    ...
    def forward(self, z, adj, Wa):
        ...
        adj_p = self.adj_process_inverse(adj)
        
        x = torch.matmul(adj_p, z + Wa) - Wa

        coo = coo_matrix(adj)
        edge_index = torch.tensor(np.array([coo.row, coo.col]), dtype=torch.float)

        x = self.gcn_shared(x, edge_index)

        mu = self.gcn(x, edge_index)
        
        logvar = self.gcn(x, edge_index)
