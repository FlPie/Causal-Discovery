...
class DAG_VGAE(pyg.nn.VGAE):
    def __init__(self, encoder, decoder):
        """
        adj_A = adjacency matrix in torch.tensor
        """
        # A = m * m
        # X = m * d, X^k = m * 1, k = 0 ~ d-1
        # Z = m * d, Z^k = m * 1, k = 0 ~ d-1
        super(DAG_VGAE, self).__init__(encoder, decoder)
    ...
    def decode(self, *args, **kwargs):
        ...
        self.__x_hat__ = self.reparametrize(self.__x_mu__, self.__x_logstd__)
    ...
    def nll_gaussian(self, x, x_hat=None, variance=0, add_const=False):
        ...
        x = x
        x_hat = self.__x_hat__ if x_hat is None else x_hat
        variance = variance
...
class DAG_GNN(pl.LightningModule):
    ...
    def forward(self, x):
        return self.model(x, self.lambda_A, self.c_A)
        
        
    def step(self, batch):
        x_hat, loss, self.adj_A_new = self.forward(batch.x)
        return x_hat, loss
    ...
    def training_epoch_end(self, outputs):
        loss      = np.sum(d['losse'] for d in outputs) / (len(outputs) * self.hparams.batch_size)
